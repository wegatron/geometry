...
#pragma omp parallel for
  ...
  void LocalVectorFieldDeformer::resolveInvalidJacobian(const std::vector<std::vector<size_t>> &indices)
  {
    ...
    while(!max_n_q.empty()) {
      ...
      std::cout << "n_ind.first=" << n_ind.first << std::endl;
    }
    abort();
    ...
    while(!max_n_q.empty()) {
      ...
      std::cout << "valid ne=" << n_ind.first << std::endl;
      ...
      std::cout << "normal scale=" << scale << std::endl;
      ...
      pplu.compute(A);
      jac_[cur_ind].block<1,3>(0,0) = pplu.solve(B[0]).transpose();
      jac_[cur_ind].block<1,3>(1,0) = pplu.solve(B[1]).transpose();
      jac_[cur_ind].block<1,3>(2,0) = pplu.solve(B[2]).transpose();
